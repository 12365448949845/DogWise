...
class X {
  #config: XConfig;
  #postprocessing: { setSize: (w: number, h: number) => void; render: () => void; dispose: () => void } | null = null;
  ...
  updateWorldSize() {
    if (this.camera.isPerspectiveCamera) {
      ...
      const cam = this.camera as unknown as { top: number; bottom: number; left: number; right: number };
      ...
      if (obj instanceof Mesh) {
        if (obj.material && typeof obj.material === 'object') {
          if ('dispose' in obj.material) {
            (obj.material as MeshPhysicalMaterial).dispose();
          }
        }
...
class W {
  ...
  reset() {
    ...
    for (let i = 0; i < config.count; i++) {
      const idx = 3 * i;
      // Cluster near center with small random offset
      positionData[idx] = MathUtils.randFloatSpread(1.5);
      positionData[idx + 1] = MathUtils.randFloatSpread(1.5);
      positionData[idx + 2] = MathUtils.randFloatSpread(1.0);
      // Give outward velocity for explosion effect
      const angle = Math.random() * Math.PI * 2;
      const elevation = (Math.random() - 0.5) * Math.PI;
      const speed = MathUtils.randFloat(0.02, 0.08);
      velocityData[idx] = Math.cos(angle) * Math.cos(elevation) * speed;
      velocityData[idx + 1] = Math.sin(elevation) * speed;
      velocityData[idx + 2] = Math.sin(angle) * Math.cos(elevation) * speed * 0.3;
    }
    this.setSizes();
  }
...
function createBallpit(canvas: HTMLCanvasElement, config: Partial<typeof DefaultConfig> = {}): CreateBallpitReturn {
  ...
  Object.assign(canvas.style, { webkitUserSelect: 'none' });
  ...
  function initialize(cfg: Partial<typeof DefaultConfig>) {
    if (spheres) {
      threeInstance.clear();
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Reset when scrolling into view
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && spheresInstanceRef.current) {
          spheresInstanceRef.current.reset();
        }
      },
      { threshold: 0.1 }
    );
